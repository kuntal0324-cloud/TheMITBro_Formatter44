...
The formatter now includes a structured question-bank ingestion layer. Text
and JPG/JPEG questions can be imported through `scripts/m32_import_question.py`.
The importer classifies each question by exam, subject, and topic and routes
high-confidence questions automatically. Ambiguous questions are placed in
`question_bank/review/` instead of being silently misclassified.
