...
class QuestionPaperRenderer:
    ...
    def _prepare_question(self, q: QuestionSpec) -> Dict[str, Any]:
        normalized = q.text if (q.metadata or {}).get("m42_print_prepared") else format_document(q.text)
    ...
    def _render_page(self, paper: PaperSpec, page_no: int, rows: Sequence[Dict[str, Any]], page_count: int) -> RenderedPage:
        ...
        svg_parts.append(self._footer_svg(paper, page_no, page_count))
    ...
    def _footer_svg(self, paper: PaperSpec, page_no: int, page_count: int) -> str:
        y = self.options.height - self.options.margin + 4
        identity = str((paper.metadata or {}).get("paper_id") or "")
        version = str((paper.metadata or {}).get("paper_version") or "")
